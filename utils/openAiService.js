...
/**
 * Parses the itinerary response from Gemini into a structured format.
 * @param {string} textResponse - The raw text response from Gemini.
 * @returns {Array} - An array of day objects with activities.
 */
const parseItineraryResponse = (textResponse) => {
  console.log("Raw Response from Gemini:", textResponse); // Log the raw response

  // Split the response by "Day X" headings
  const days = textResponse.split(/\*\*Day \d+ \(.*?\)\*\*/).slice(1);
  console.log("Days extracted:", days); // Log the extracted days

  return days.map((dayText, index) => {
      const lines = dayText.trim().split("\n").filter(line => line.trim() !== '');
      console.log("Lines for Day:", lines); // Log the lines for each day
      ...
      lines.forEach(line => {
          if (line.startsWith("- **")) {
              ...
              currentActivity = {
                  timeSlot: line.match(/\*\*(.*?)\*\*/)[1] // Extract "Morning Activity", "Midday Activity", etc.
              };
          } else if (line.includes("Title:")) {
              currentActivity.title = line.split("Title:")[1].trim();
          } else if (line.includes("Budget:")) {
              currentActivity.budget = line.split("Budget:")[1].trim();
          } else if (line.includes("Description:")) {
              currentActivity.description = line.split("Description:")[1].trim();
          } else if (line.includes("Tips:")) {
              currentActivity.tips = line.split("Tips:")[1].trim();
          } else if (line.includes("Duration:")) {
              currentActivity.duration = line.split("Duration:")[1].trim();
      ...
      return {
          days: `Day ${index + 1}`,
          dayTitle: `Day ${index + 1}`, // You can extract the actual day title if needed
          activities
      };
  });
};
...
exports.getActivitySuggestions = async (details) => {
  try {
      console.log("This is the details received to Gemini :: ", details);
      ...
      // Parse and format the response
      const formattedResponse = parseItineraryResponse(textResponse.trim());
